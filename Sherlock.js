...
export class Sherlock{
    ...
    init(string, tools){
        this.string=string;
        this.opStack.push({'op#':this.opStack.length-1, 'finding':undefined, 'op': 'Sherlock', 'tools':tools, 'page':this.pageQueue[0]});
        this.ugly_itr=0;
        return this;
    }
	nextPage(){
    ...
    nextFinding(cb, tools){
        ...
        tools['patterns'].forEach((pattern)=>{
            var finding = new Finding().next(this.opStack[this.opStack.length-1]['page'], pattern)
            //if there is no finding there is no reason to iterate the same function
